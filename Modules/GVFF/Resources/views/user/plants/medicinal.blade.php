...
@section('content')
    <section class="mt-20" data-aos="fade-up">
        <h2 class="text-3xl font-bold text-green-800 text-center mb-6">Plantas Medicinales</h2>
                ...
                @foreach($plants as $plant)
                    @if($plant->available)
                        <div class="bg-white rounded-xl shadow-lg p-6 transform hover:scale-105 transition-all duration-300" data-aos="zoom-in">
                            ...
                            <p class="text-gray-600 mb-2"><strong>Descripción:</strong> {{ $plant->characteristics ?? 'Sin descripción' }}</p>
        ...
        @else
            <p class="text-gray-600 text-center">No hay plantas medicinales disponibles en este momento.</p>
        @endif
    </section>
@endsection
